...
event_file='/home/idies/erdata/data/pm00_300003_020_EventList_c001.fits'

mkdir -p data/full_eband && mkdir -p data/gti_files\
    && mkdir -p data/light_curves && mkdir -p data/masks

gti_dir="data/gti_files/"
curves_dir="data/light_curves"
...
for ((i = 0; i < len; i++)); do
    ...
    evtool eventfiles="$event_file"\
            ...
            emin=0.2 emax=10. image=yes size=1250 center_position=auto   
    ...
    flaregti eventfile="$out_dir/reduced_TM${TMs[$i]}.fits"\
        ...
        lightcurve="$curves_dir/lightcurve_6_10_keV_TM${TMs[$i]}.fits" \
        mask="$mask_dir/mask_6_10_keV_TM${TMs[$i]}.fits" \
        timebin=100 pimin=6000 pimax=10000  #10 keV
    ...
    flaregti eventfile="$out_dir/reduced_TM${TMs[$i]}.fits"\
        ...
        threshold="${thresh[$i]}" \
        timebin=100 pimin=6000 pimax=10000
...
for ((i = 0; i < len; i++)); do
        evtool eventfiles="$inp_dir/reduced_TM${TMs[$i]}.fits"\
                ...
                emin=0.3 emax=2. image=yes size=1250 center_position=auto
done
...
for ((i = 0; i < len; i++)); do
        evtool eventfiles="$inp_dir/reduced_TM${TMs[$i]}.fits"\
                outfile="$out_dir/filtered_TM${TMs[$i]}.fits"\
                telid=${TMs[$i]} flag=0xc00fff30 pattern=15 gti="FLAREGTI" \
                emin=1. emax=2. image=yes size=1250 center_position=auto 
done